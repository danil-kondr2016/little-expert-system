 ...
 *
 * <p>База знаний представляет
 * собой текстовый файл (который в дальнейшем может быть зашифрован), включающий
 * три секции со следующей структурой:</p>
 *
 * <p><b>1.</b></p>
 *
 * <p>Описание базы знаний, имя автора, комментарий и т.п.</p>
 *
 * <p>(можно в несколько строк, общая длина которых не должна
 * превышать 10000 символов; данная секция заканчивается после первой пустой
 * строки).</p>
 *
 * <p><b>2.</b></p>
 *
 ...
 *
 * <p>Смысл первых двух секций
 * вполне понятен из приведённой схемы. Последняя секция требует более подробного
 * рассмотрения. В ней перечисляются правила вывода: каждое задаётся в отдельной
 * строке; перечисление заканчивается с концом файла.</p>
 *
 * <p>В начале описания правила
 * вывода задаётся исход, вероятность которого меняется в соответствии с данным
 * правилом. Это текст, включающий любые символы, кроме запятых. После запятой
 * указывается априорная вероятность данного исхода (P), т.е. вероятность исхода в
 * случае отсутствия дополнительной информации. После этого через запятую идёт ряд
 * повторяющихся полей из трёх элементов. Первый элемент (i) – это номер соответствующего
 * вопроса (симптома, свидетельства). Следующие два элемента ( Py = P(E / H) и Pn
 * = P(E / неH) ) – соответственно вероятности получения ответа «Да» на этот
 * вопрос, если возможный исход верен и неверен. Эти данные указываются для
